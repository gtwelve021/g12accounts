...
import { sectionClass } from "@/components/ui-classes";
...
export default function PrivacyPage(){return <main><PageHero index="05" label="Privacy" title={<>Privacy<br/><em>Policy.</em></>} description="Information about how website data may be collected, used and retained."/><section className={sectionClass}><SectionLabel number="01">Website privacy</SectionLabel><div className="mt-[65px] grid items-start gap-[clamp(45px,9vw,130px)] min-[801px]:grid-cols-[220px_1fr]"><aside className="border-t border-[#07152f29] pt-[22px] text-[11px] leading-[1.7] text-[#66718a] min-[801px]:sticky min-[801px]:top-[110px]"><p>G12 Accounts website policy</p><a className="mt-5 inline-block font-bold uppercase tracking-[.08em] text-[#2447a8]" href="mailto:finance@g12.ae">Privacy enquiries ↗</a></aside><div>{sections.map((section,index)=><section className="mb-[50px] grid grid-cols-[32px_1fr] gap-2.5 border-b border-[#07152f29] pb-[50px] min-[521px]:grid-cols-[50px_1fr] min-[521px]:gap-5" key={section.title}><span className="pt-2.5 text-[9px] text-[#2447a8]">{String(index+1).padStart(2,"0")}</span><div><h2 className="mb-[22px] text-[clamp(30px,3.2vw,45px)]">{section.title}</h2>{section.body.map(text=><p className="mb-4 leading-[1.85] text-[#66718a]" key={text}>{text}</p>)}</div></section>)}</div></div></section></main>}
